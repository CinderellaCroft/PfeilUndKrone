...
const genId = p => `${p}-${Date.now()}-${(Math.random() * 1e3 | 0)}`;
...
function createInitialGameState() {
  return {
    ...
    turnNumber: 0,
    isGameOver: false,
    king:   { ws: null, resources: { gold: 499, wood: 50, grain: 200 }, submittedPaths: [], purchasedWorkers: 0 },
...
function startNewRound(lobby) {
  ...
  sendTo(lobby, 'King', { type: 'new_round', payload: { roundNumber: gs.turnNumber, resources: gs.king.resources, workers: gs.king.purchasedWorkers } });
...
function evaluatePaths(gs) {
  ...
  //kingsPath in ungerichtete edges umwandeln und ambush treffer/distanz ermitteln
  ...
  intercepts.forEach((intercept, index) => {
    ...
    if (intercept.resource != null) {
      ...
      console.log("debug intercept.resource.resource.", resource)
      if (resource != null) {
        switch (resource) {
          case "Wheat":
            banditBonus.wheat += distance * multiplier;
            break;
          case "Ore":
            banditBonus.ore += distance * multiplier;
            break;
          case "Wood":
            banditBonus.wood += distance * multiplier;
            break;
        }
      }
    } else {
      resource = originResourceFields[index].resource;
      console.log("kings resource at origin in intercepts.forEach: ", resource)
      switch (resource) {
        case "Wheat":
          kingBonus.wheat += distance * multiplier;
          break;
        case "Ore":
          kingBonus.ore += distance * multiplier;
          break;
        case "Wood":
          kingBonus.wood += distance * multiplier;
          break;
      }
    }
  });
  ...
  return { outcome: outcomes, winner };
}
